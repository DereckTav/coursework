...
author: "Dereck Taverne (CUNY / Baruch College)"
date: '`r Sys.time()`'
output:
  html_document:
    toc: yes
...
```{r load-counts}
year_counts <- read.table(here("week3/ngrams/year_counts.tsv"),
  col.names = c("term", "year", "volume", "book_count")
)

total_counts <- read.csv(here("week3/ngrams/total_counts.csv"),
  col.names = c("year", "total_volume", "page_count", "book_count")
)
```
...
``` {r length-of-data}
year_counts |> summarize(number_of_rows = nrow(year_counts))
total_counts |> summarize(number_of_rows = nrow(total_counts))
```
...
joined_data <- year_counts |>
  filter(term %in% c(1883, 1910, 1950)) |>
  left_join(total_counts, by = join_by(year)) |>
  filter(!is.na(total_volume)) |>
  mutate(frequency = volume / total_volume * 10000) |>
...
joined_data |> ggplot(aes(x = year, y = frequency, color = term)) +
  geom_line(size = 2) +
  coord_cartesian(xlim = c(1852, 2003)) +
...
Add your screenshot for Part C below this line using the `![](Screenshot 2025-06-13 154439.png)` syntax and comment on similarities / differences.
...
joined_data |> ggplot(aes(x = year, y = volume, color = term)) +
  geom_line(size = 2) +
...
```{r plot-totals}
total_counts |> ggplot(aes(x = year, y = total_volume)) +
  geom_line(size = 2) +
  coord_cartesian(xlim = c(1850, 2012)) +
...
min_years <- joined_data |>
  group_by(term) |>
  filter(frequency >= max(frequency) / 2) |>
  summarize(term, year) |>
  count(term)

max_year_per_term |>
  mutate(halflife = min_years$n) |>
  View()
```
